...
export function assertContentPrefix(
	value: string,
): asserts value is ContentPrefix {
	if (!isContentPrefix(value))
		throw new TypeError(`invalid ContentPrefix "${value}"`);
}
...
export abstract class Backend {
	...
	abstract read(
		id: ContentId,
		signal?: AbortSignal,
	): Promise<null | { content: Buffer<ArrayBufferLike>; type: MimeType }>;
	abstract write(
		id: ContentId,
